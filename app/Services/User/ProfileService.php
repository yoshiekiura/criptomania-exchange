...
class ProfileService
{
    ...
    public function updatePassword(PasswordUpdateRequest $request)
    {
       ...
       public function createBank(BankNameRequest $request)
    {
        // $create = ['password' => Hash::make($request->new_password)];
        $create = $request->only('users_id','bank_name','account_number');

         if (app(BankNameInterface::class)->insert($create)) {
            $notification = ['user_id' => Auth::id(), 'data' => __("The Bank Name was successfully created.")];
            app(NotificationInterface::class)->create($notification);

            return [
                SERVICE_RESPONSE_STATUS => true,
                SERVICE_RESPONSE_MESSAGE => __('successfully Created.')
            ];
        }
        ...
        return [
            SERVICE_RESPONSE_STATUS => false,
            SERVICE_RESPONSE_MESSAGE => __('Failed To Create Bank Name. Please Try Again')
        ];
    }
