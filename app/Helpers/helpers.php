...
if (!function_exists('get_struck')) {
    function get_struck($struck)
    {
        $struckPath = 'storage/' . config('commonconfig.path_struck_upload');

        $struck = valid_image($struckPath, $struck) ? $struckPath . $struck : $struckPath . 'avatar.jpg';
...
if (!function_exists('get_transaction_type')) {
    function get_transaction_type($key = null)
    {
        $transactionType = [
            ...
            TRANSACTION_TYPE_TRANSFER => __('Bank Transfer')
        ];
