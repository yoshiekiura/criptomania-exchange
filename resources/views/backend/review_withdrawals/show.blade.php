                ...
                <div class="box-body">
                    <div class="form-horizontal show-form-data">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="hidden" class="form-control" value="{{ $withdrawal->amount }}" name="amount">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                @if($withdrawal->payment_method == BANK_TRANSFER)
                                <p class="form-control-static"> <span class="well well-sm">{{$bankName}} {{ $withdrawal->address }}</span></p>
                                @else
                        ...
                        <div class="col-md-12 text-right">
                            @if($withdrawal->status == PAYMENT_REVIEWING)
                                @if($withdrawal->payment_method == BANK_TRANSFER)
                                    ...
                                    <a href="{{ route('admin.review-withdrawals.approveBank', ['id' => $withdrawal->id]) }}" class="btn btn-sm btn-primary btn-flat btn-sm-block confirmation" data-form-id="approve-{{ $withdrawal->id }}" data-form-method="PUT" data-alert="{{__('Do you want to approve this withdrawal?')}}">{{ __('Approve WD Bank') }}</a>
                                    @endif
                                ...
                                @if($withdrawal->payment_method == BANK_TRANSFER)
                                    ...
                                    @if( has_permission('admin.review-withdrawals.declineBank') )
                                        <a href="{{ route('admin.review-withdrawals.declineBank', ['id' => $withdrawal->id]) }}" class="btn btn-sm btn-danger btn-flat btn-sm-block confirmation" data-form-id="decline-{{ $withdrawal->id }}" data-form-method="PUT" data-alert="{{__('Do you want to decline this withdrawal?')}}">{{ __('Decline WD Bank') }}</a>
                                    @endif
