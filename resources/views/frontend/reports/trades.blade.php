                            ...
                            <table class="table datatable dt-responsive display nowrap dc-table"
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @endif
                                        <td>{{ $transaction->price }} <span
                                                class="strong">{{ $transaction->base_item_abbr }}</span></td>
                                        <td>{{ $transaction->amount }} <span
                                                class="strong">{{ $transaction->stock_item_abbr }}</span></td>
                                        <td>
                                            {{ bcadd($transaction->fee,$transaction->referral_earning) }}
                                            <span
                                                class="strong">{{ $transaction->exchange_type == EXCHANGE_BUY ? $transaction->stock_item_abbr : $transaction->base_item_abbr }}</span>
                                            ({{ $transaction->is_maker == 1 ?
                                            number_format($transaction->maker_fee, 2) . '%' :
                                            number_format($transaction->taker_fee, 2) . '%' }})
                                        </td>
                                        <td>{{ $transaction->total }} <span
                                                class="strong">{{ $transaction->base_item_abbr }}</span></td>
