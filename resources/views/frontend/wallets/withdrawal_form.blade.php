...
@section('content')
@php
$bankAddress = App\Models\User\BankName::select('account_number','bank_name')->where('users_id',Auth::user()->id)->get();
            ...
            <div class="box box-primary box-borderless">
                @if($wallet->stockItem->withdrawal_status == ACTIVE_STATUS_ACTIVE)
                    ...
                    <div class="box-body">
                        ...
                        {!! Form::open(['route'=>['trader.wallets.withdrawal.store', $wallet->id], 'method' => 'post', 'class'=>'form-horizontal validator']) !!}
                        ...
                        @if($wallet->stockItem->item_type == CURRENCY_CRYPTO)
                            ...
                            <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <p class="help-block">
                                        {!! __('Minimum withdrawal amount :amount :stockItem', [ 'amount' => '<span class="text-info">' . $wallet->stockItem->minimum_withdrawal_amount . '</span>', 'stockItem' => $wallet->stockItem->item]) !!}
                                    </p>
                        ...
                        @elseif($wallet->stockItem->item_type == CURRENCY_REAL)
                            ...
                            @if($wallet->stockItem->api_service == BANK_TRANSFER)
                             ...
                             {{--address--}}
                            <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                                <label class="col-md-4 control-label required">{{ __('Select Your Bank address') }}</label>
                                <div class="col-md-8">
                                     <select class="form-control" id="{{fake_field('address')}}" data-cval-name="The address field" data-cval-rules="required|numeric" name="{{fake_field('address')}}" placeholder="{{__('Select Bank')}}">
                                        <!-- <option selected>Please select a Bank</option> -->
                                        @foreach($bankAddress as $bank)
                                            <option value="{{$bank->account_number}}">{{$bank->bank_name}} {{$bank->account_number}}</option>
                                        @endforeach
                                    </select>
                                    <span class="validation-message cval-error"
                                          data-cval-error="{{ fake_field('address') }}">{{ $errors->first('address') }}</span>
                            ...
                            {{--amount--}}
                            <div class="form-group {{ $errors->has('amount') ? 'has-error' : '' }}">
                                <label for="{{ fake_field('amount') }}" class="col-md-4 control-label required">{{ __('Amount Bank') }}</label>
                                <div class="col-md-8">
