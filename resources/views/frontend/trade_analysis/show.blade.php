                ...
                <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5">
                            ...
                            <div><i class="fa fa-pencil-square-o"></i> {{$post->user->userInfo->full_name}}<span
                                    style="margin:0 5px;"></span><i class="fa fa-clock-o"></i>
                                {{$post->created_at}}<span style="margin:0 5px;"></span><i class="fa fa-comment-o"></i>
                                {{$post->comments->count()}}</div>
                            <p style="margin-top:30px">{!! $post->content !!}</p>
                            <div style="padding-top:40px">
                                <h2 class="title-center cm-mt-5 cm-mb-15">{{__('Comments')}}
                                    ({{$post->comments->count()}})</h2>
                                <div class="chat">
                                    @forelse($post->comments->sortByDesc('id') as $comment)
                                    <div class="item">
                                        <img src="{{get_avatar($comment->user->avatar)}}" alt="user image"
                                            class="offline" style="border:none; border-radius:0">
                                ...
                                <textarea class="form-control" name="{{fake_field('content')}}"
                                    rows="3">{{old(fake_field('content'))}}</textarea>
                                {{ Form::submit(__('Submit Comment'), ['class'=>'btn btn-success form-submission-button cm-mt-10']) }}
                                {!! Form::close() !!}
                                @endauth
                                @guest
                                <p>{!! __('Please :login to comment this post',['login'=>'<a
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="pad-tb-20 text-center">
                                        <img src="{{asset('frontend/images/footer.png')}}" alt=""
